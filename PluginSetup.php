...
class PluginSetup {
	...
	public static function init() {
		$inc_dir = NOLTERESOURCECENTERPLUGIN_PLUGIN_DIR . 'src';

		$modules_dir = NOLTERESOURCECENTERPLUGIN_PLUGIN_DIR . 'src/Modules';
	...
	public static function maybe_deactivate() {
		...
		load_plugin_textdomain( NOLTERESOURCECENTERPLUGIN_TEXT_DOMAIN );

		require_once( ABSPATH . 'wp-admin/includes/plugin.php' );

		if ( version_compare( $wp_version, NOLTERESOURCECENTERPLUGIN_MINIMUM_WP_VERSION, '<' ) ) {

			$msg = sprintf( __(
				'Plugin %s requires WordPress %s or higher.',
				NOLTERESOURCECENTERPLUGIN_TEXT_DOMAIN
			), NOLTERESOURCECENTERPLUGIN_PLUGIN_VERSION, NOLTERESOURCECENTERPLUGIN_MINIMUM_WP_VERSION );
